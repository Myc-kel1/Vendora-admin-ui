...
const addToRemoveQueue = (id) => { if (toastTimeouts.has(id)) return; const t = setTimeout(() => { toastTimeouts.delete(id); dispatch({ type: 'REMOVE_TOAST', toastId: id }) }, TOAST_REMOVE_DELAY); toastTimeouts.set(id, t) }
...
const listeners = []; let memoryState = { toasts: [] }
function dispatch(action) { memoryState = reducer(memoryState, action); listeners.forEach((l) => l(memoryState)) }
...
export function useToast() {
  ...
  useEffect(() => { listeners.push(setState); return () => { const i = listeners.indexOf(setState); if (i > -1) listeners.splice(i, 1) } }, [state])
